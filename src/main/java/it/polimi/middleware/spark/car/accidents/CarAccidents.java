...
public class CarAccidents {
	public static void main(String[] args) {
		...
		// FILTERING OF INCORRECT VALUES NUM_PERSON must be equals to SUM(N OF
		// PEDESTRIANS, N_OF_CYCLIST, N_OF_MOTORIST)
		...
		final Dataset<Row> ds_with_correct_nums = ds
				.withColumn("TOTAL_I",
						ds.col("NUMBER OF PEDESTRIANS INJURED")
								.plus(ds.col("NUMBER OF CYCLIST INJURED").plus(ds.col("NUMBER OF MOTORIST INJURED"))))
				...
				.withColumn("TOTAL_K",
						ds.col("NUMBER OF PEDESTRIANS KILLED")
								.plus(ds.col("NUMBER OF CYCLIST KILLED").plus(ds.col("NUMBER OF MOTORIST KILLED"))))
		...
		// We should filter the data set and take only accidents that have the correct
		// values
		final Dataset<Row> ds_corrected = ds_with_correct_nums
				.filter(ds_with_correct_nums.col("FAKETOTAL_I").equalTo(ds_with_correct_nums.col("TOTAL_I"))
		...
		// We are interested in group by YEAR and WEEK so we should create those two
		// columns and sum the total number of accidents
		final Dataset<Row> ds_le_per_week = ds_lethal_accidents
				.withColumn("WEEK", weekofyear(to_date(ds_lethal_accidents.col("DATE"), "MM/dd/yyyy")))
		...
		// Q2 Number of accidents and percentage of number of deaths per contributing
